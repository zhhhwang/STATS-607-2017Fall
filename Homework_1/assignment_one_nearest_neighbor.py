...
def get_fold_indices(n, num_folds, fold_id):
    ...
    if fold_id == num_folds-1:
        ...
        train_indices = range(0, fold_id*fold_size-1) + range((fold_id+1)*fold_size, n)

    # TASK 3.1.3
    # set test_indices to those indices in range(n)
    # that are not in train_indices
    test_indices = [i for i in range(0, n) if i not in train_indices]
...
def nn_classifier(point, train_data, train_labels, k, K):
    ...
    # put distances, training examples and training labels together
    all_three = zip(distances, train_data, train_labels)

    # TASK 3.2.1
    # sort the triples by distances
    all_three_sorted = [sorted(all_three, key=lambda all_three: all_three[0])]

    # TASK 3.2.2
    # after sorting extract the labels from the first k
    # triples
    nearest_k_labels = [all_three_sorted[0][i][2] for i in range(k)]
    ...
    for label in range(K):
        freq[label] = len([i for i in nearest_k_labels if i==label])
...
def main():
    ...
    for k in k_range:
        ...
        for fold_id in range(num_folds):
            ...
            classifier = lambda x: nn_classifier(x, train_data, train_labels, k, K)
