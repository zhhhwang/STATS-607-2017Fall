...
def kmeans_plus_plus(instances, K):
    ...
    while len(centers) < K:
        probs = []
        distance.append([min(euclidean_squared(x, c) for c in centers) for x in instances])
        sum_distance = sum(distance[0])
        probs = [x / sum_distance for x in distance[0]]

        r = random.random()
        cumulative_probability = 0.0
        for cases, case_probs in zip(instances, probs):
            cumulative_probability += case_probs
            if r < cumulative_probability:
                centers.append(cases)
                break
    return centers
...
def assign_cluster_ids(instances, centers):
    ...
    for i in range(n):
        ...
        # above.
        distances = [euclidean_squared(instances[i], c) for c in centers]
...
def recompute_centers(instances, cluster_ids, centers):
    ...
    for i in range(K):
        ...
        one_cluster = [index for index, cases in enumerate(cluster_ids) if cases == i]
        ...
        LIST = [instances[k] for k in one_cluster]
        sum_cluster = reduce(lambda x, y: [sum(z) for z in zip(x, y)], LIST)
