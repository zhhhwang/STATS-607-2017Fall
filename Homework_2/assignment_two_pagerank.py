...
def compute_pageranks(google_mat, tol=1e-8):
    ...
    while rel_change > tol:
        ...
        new_dist = np.dot(dist, google_mat)
...
def main():
    ...
    row_sum = adj_mat.sum(axis=1)
    row_sum[row_sum[:] == 0] = 1
    adj_mat_norm = adj_mat / row_sum[:, np.newaxis]
    ...
    top_10 = sorted(range(len(pageranks)), key=lambda k: -pageranks[k])[:10]
    ...
    close_to_1 = np.array(np.ndarray.nonzero(np.isclose(w, np.ones(n))))
