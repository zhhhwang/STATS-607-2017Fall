...
def svm_train(instances, labels, kernel_func, C=1.0):
    ...
    def func(alpha):
        ...
        func_value = - np.sum(alpha) + 0.5 * alpha.dot(kernel_mat * np.outer(pm_labels, pm_labels)).dot(alpha)
    ...
    def func_deriv(alpha):
        ...
        func_deriv_value = - np.ones(n) + (kernel_mat * np.outer(pm_labels, pm_labels)).dot(alpha)

        return func_deriv_value
    ...
    def classifier(point):
        ...
        inner_products = np.array([kernel_func(point, x) for x in support_vectors])
        given_label = alpha_y_nz.dot(inner_products)
        return given_label > 0
...
def evaluate_classifier(classifier, instances, labels):
    ...
    pos_correct = sum(pos_predictions == True)
    ...
    neg_correct = sum(neg_predictions == False)
